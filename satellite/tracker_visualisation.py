...
if satellite_data:
    ...
    earth = sphere(pos=vector(0,0,0), radius=6378, texture=textures.earth) #Earth's radius is ≈ 6378 km
    satellite_visual = sphere(radius=13, color=color.green, make_trail=True, trail_radius = 5, trail_color=color.red)
    info_label = label(pos=satellite_visual.pos, text = '', height=16, border=3, font='sans')
    ...
    while True:
        ...
        satellite_visual.pos = vector(x,y,z)
        info_label.pos = satellite_visual.pos + vector(0,50,0)
        ...
        speed = (velocity[0]**2 + velocity[1]**2 + velocity[2]**2)**0.5 #I converted velocity into a format I can use
